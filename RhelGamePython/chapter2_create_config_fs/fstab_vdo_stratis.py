    ...
    try:
        ...
        while True:
            ...
            if user_input.strip() == "echo '/dev/mapper/vdo1 /mnt/vdo xfs defaults,x-systemd.requires=vdo.service 0 0' >> /etc/fstab":
                ...
                slow_validInput.print_slow("Hint: echo '/dev/mapper/vdo1 /mnt/vdo xfs defaults,x-systemd.requires=vdo.service 0 0' >> /etc/fstab")
                continue
...
def add_stratis_fstab():
    ...
    try:
        ...
        while True:
            user_input = input("Type the command to add a Stratis filesystem to /etc/fstab: ")
            ...
            if user_input.strip() == "echo 'stratispool /mnt/stratis xfs defaults,x-systemd.requires=stratisd.service 0 0' >> /etc/fstab":
                ...
                slow_validInput.print_slow("\nAdditional Information:")
                slow_validInput.print_slow("- 'stratispool-mnt': Mount point for the Stratis filesystem")
                slow_validInput.print_slow("- '/mnt/stratis': Mount point for the Stratis filesystem")
                slow_validInput.print_slow("- 'xfs': Filesystem type")
                slow_validInput.print_slow("- 'defaults': Default mount options")
                slow_validInput.print_slow("-  'x-systemd.requires=vdo.service': Default mount options with systemd requirement")
                slow_validInput.print_slow("- '0 0': Filesystem check and backup options")
                return True
            else:
                slow_validInput.print_slow("Incorrect command. Please try again or type 'quit' to exit.")
