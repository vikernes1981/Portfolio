...
def provide_uuid_line():
    ...
    slow_validInput.print_slow("\n\nAs you journey through the digital wilderness, you stumble upon a mysterious path.\n\n"
                               ...
                               f"Your quest begins with the discovery of the following UUID: {correct_uuid}\n")
    
    while True:
        user_line = input("Enter the line with the UUID: ")
        print("\n")
        if user_line.strip() in quit_command:
...
def edit_fstab():
    ...
    while True:
        edit_command = input("Navigate to fstab file or edit it directly using vim : ")

        if "cd /etc/fstab" in edit_command:
            ...
            if "vim" in vim_command and ("fstab" in vim_command or "/etc/fstab" in vim_command):
